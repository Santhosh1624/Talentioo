...
const TodoList = () => {
  ...
  function create() {
    axios.post('http://localhost:5001/posting', { todo })
      .then(() => {
  ...
  function getData() {
    axios.get('http://localhost:5001/getting')
      .then((response) => {
  ...
  const updatedTodo = (id, updatedData) => {
    axios.put(`http://localhost:5001/updating/${id}`, { todo: updatedData })
      .then(() => {
  ...
  function deleteTodo(id) {
    axios.delete(`http://localhost:5001/deleting/${id}`)
      .then(() => {
        getData();
      })
      .catch(() => {
